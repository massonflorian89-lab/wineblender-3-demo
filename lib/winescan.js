...
(function (global) {
  ...
  const MAPPED = new Set(['D','TAV','pH','AT','VA 111','SO2 L','SO2 T','SO2 Actif','SucTot','Sucre','Malic','ALac','CO2','GF-5','GF+5']);
  ...
  function toAnalyse(row, derived){
    ...
    return {
      ...
      so2_libre: _num(row['SO2 L']), so2_total: _num(row['SO2 T']), so2_actif: _num(row['SO2 Actif']),
      sucres_residuels: suc,
      acide_malique: _num(row.Malic), acide_lactique: _num(row.ALac), co2: _num(row.CO2),
      gf_neg5: _num(row['GF-5']), gf_pos5: _num(row['GF+5']),
